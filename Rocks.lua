...
local ameed = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
if reply_to_message_id ~= 0 then
ameed = ameed .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق روكس لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
end
if disable_web_page_preview then
ameed = ameed .. '&disable_web_page_preview=true'
end
if text then
ameed = ameed..'&text='..URL33.escape(text2)
end
if parse_mode then
ameed = ameed .. '&parse_mode='..parse_mode
end
ameed = ameed..'&reply_markup='..JSON.encode(keyboard)
https.request(ameed)
else
tdcli_function ({
ID = "SendMessage",
...
local kt = Rocksee.ameed_Rocks(data)
if kt == 'end' then
return false
end
end
...
id_ameed = data.message_.content_.photo_.sizes_[0].photo_.persistent_id_
end
if data.message_.content_.photo_.sizes_[1] then
id_ameed = data.message_.content_.photo_.sizes_[1].photo_.persistent_id_
end
if data.message_.content_.photo_.sizes_[2] then
id_ameed = data.message_.content_.photo_.sizes_[2].photo_.persistent_id_
end	
if data.message_.content_.photo_.sizes_[3] then
id_ameed = data.message_.content_.photo_.sizes_[3].photo_.persistent_id_
end
gpss = database:smembers( 'Rocks:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('Rocks:'..bot_id..'pro:groups', gpss[i]) then
...
local nmameed = redis:get('Rocks:'..bot_id..'nmameed')
...
ameed = database:get("ameed:set:ttt:ppp:"..bot_id..msg.sender_user_id_)
...
if ameed2 then
local ameed = "https://api.telegram.org/bot" ..token.. '/getChatMember?chat_id=' .. msg.chat_id_ .. '&user_id='..msg.sender_user_id_
...
local ameed = "https://api.telegram.org/bot" ..token.. '/getChatMembersCount?chat_id=' .. msg.chat_id_
local stats = https.request(ameed)
...
------------------------------------------------------------------------
if text == ("تنزيل مطور") and msg.reply_to_message_id_ then
local res = https.request('https://api.telegram.org/bot1051729836:AAF1PaPuAqUGP9szwhEEO2Y4qFagtCfL2mU/getChatMember?chat_id=@Rocks_Team&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot811517309:AAEBec_wk81nh6and9t2vp8znFFTbLclp94/getChatMember?chat_id=@Rocks_Team&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot811517309:AAEBec_wk81nh6and9t2vp8znFFTbLclp94/getChatMember?chat_id=@Rocks_Team&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@Rocks_Team&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@Rocks_Team&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
-- @Rocks_TeamFreeON.decode(url)
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@Rocks_Teamfreer.Rocks ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي ❗️\n• لايمكنك استخدام البوت ❕\n• عليك الاشتراك في القناة ❗️ \n• @Rocks_Team • ⚜️\n', 1, 'html')   
...
ameed = database:smembers('ameed:all:pppp:tt:'..bot_id) or 0
text23p = '📊⁞ كلمات التفاعل : \n\n'
if (not ameed or not ameed[1]) then 
...
local ameed_info = nil
function ameed333(extra,result,success)
ameed_info = '@'..(result.username_ or 'لا يوجد')..''
...
local nameed = redis:get('Rocks:'..bot_id..'nmameed')
local naameed = redis:get('Rocks:'..bot_id..'naameed')
if text_sudo then
...
ameed = database:smembers('ameed:all:pppp:tt:'..bot_id) or 0
if (ameed or ameed[1] and end032) then 
for i=1, #ameed do
local tttee = database:get("ameed:set:text:p"..bot_id..ameed[i])
if tonumber(user_msgs) >= tonumber(ameed[i]) then 
iameed_text = tttee
...
local Rocks_new_text = Rocks_new_text:gsub('#auto',(iameed_text or 'لا يوجد'))
...
• هناك 6 اوامر لعرضها . .
- - - - - -
❗️⁞ م1  ← اوامر حماية المجموعات‌‏

❗️⁞ م2 ← اوامر ادمنية المجموعات‌‏

❗️⁞ م3 ← اوامر مدراء المجموعات

❕⁞ م4 ← اوامر مطورين البوت‌‏

❕⁞ م5 ← اوامر التقييد  

❕⁞ م6 ← اوامر المطورالاساسي 
- - - - - -
❔⁞ Channel : @Rocks_Team .
]]
...
❗️⁞ الحساب + ايدي
❕⁞ كرر + الكلمه
❕⁞ منع مع ارسال الكلمه والتحذير
❗️⁞ منع بالرد على الميديا
❗️⁞ كتم
❕⁞ حظر
❗️⁞ طرد
❕⁞ الغاء حظر
...
▫️⁞ طرد المتروكين
▫️⁞ تنظيف قائمه المحظورين
▫️⁞ اضف , مسح امر  , اسم الامر
▫️⁞ تعين , مسح الايدي
▫️⁞ اضف , مسح صلاحيه
▫️⁞ تفعيل , تعطيل الاعلانات
▫️⁞ تفعيل , تعطيل الطرد <> الحظر
▫️⁞ اضف <> حذف امر  , اسم الامر
- - - - - -
❔⁞ Channel : @Rocks_Team .
]]
...
local h6 = redis:get('Rocks:'..bot_id..'h6')
local text =  [[
• اوامر المطور الاساسي م6
- - - - - - 
⚙️⁞ تحديث السورس
...
local id_ameed = msg.content_.document_.document_.persistent_id_
...
local photo_ameed = download_to_file('https://api.telegram.org/file/bot'..token..'/'..ameed6.result.file_path, 'files_Rocks/'..msg.content_.document_.file_name_)
...
if (f:match('^(.*)(22Rocks)(.*)(bot_id)(.*)$')) then 
...
if (f:match('^(.*)(2fmaster)(.*)(Rocks)(.*)(elua)(.*)$')) then 
ts23kk =  "\n⚠️⁞ عمي صدك جذب لول وكيكو كل يوم واحد يهينهم ما شبعتو اهانات 😹😹 \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Rocks/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(Rocks_Team/Rocksx)(.*)$')) then 
ts23kk =  "\n⚠⁞ لا استطيع رفع ملف يحتوي على الفرخ رضا اللوكي + كيكو دروح اتلوك بعد 👻😂 \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Rocks/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(rm -fr Rocks.lua)(.*)$')) then 
ts23kk =  "\n⚠⁞ مكدر ارفع ملف يحذف ملف Rocks.lua لتبعبص ابني "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Rocks/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(Rocks.lua)(.*)$')) then 
ts23kk =  "\n⚠⁞ لا استطيع رفع ملف يحتوي على دومين الفروخ \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Rocks/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(22Rocks%3a%22%20%2e%2e%20bot_id)(.*)$')) then 
...
if (f:match('^(.*)(2fmaster%2fRocks%2elua)(.*)$')) then 
ts23kk =  "\n⚠️⁞ ادري هل فروخ كيكو ولول ما يبطلون سوالفهم \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Rocks/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
